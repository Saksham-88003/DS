...
if (first <= last)
{
int mid = (first + last) / 2; if (el == arr[mid])
return mid;
else if (el < arr[mid])
binarySearch(arr, el, first, mid - 1); else if (el >
arr[mid])
...
int arr[n];
cout << "Enter Elements of Array: "; for (int i = 0; i < n;
i++)
cin >> arr[i]; int el;
cout << "Enter Element to Search: "; cin >> el;
cout << "RESULT: ";
int index = binarySearch(arr, el, 0, n - 1); if (index == -
1)
